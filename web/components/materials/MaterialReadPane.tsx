...
export function MaterialReadPane({
  ...
  // PDF ビューア中核 (Phase 3 モノリス分割: usePdfViewer に抽出)。
  // ロード (L1 → Storage 復元) / ページ / 見開き / ズーム / フィット描画 / ページ移動。
  const {
    loaded,
    numPages,
    page,
...
4) 最後に「まずは一度ざっと通して読んでみよう。分からない所は飛ばして大丈夫。読み終えて『レジュメにする』を押したら、概念ごとに一緒にまとめていこうね。ここから始めていい?」と通読を促し、軽く確認する。`
    ...
    [loaded, starting, sending, pagesToShow, packPages, material, subject, page, history, streamAokiReply, setPage],
  );
  ...
  const explainGuidedBlock = useCallback(
    ...
    [loaded, spread, packPages, material, subject, history, assignmentMode, streamAokiReply, setPage],
  );
  ...
  const commitGuidedBbox = useCallback(
    ...
    [guidedSegment, guidedIndex, pagesToShow, spread, persistGuidedPlans, canvasRefs, setPage],
  );
