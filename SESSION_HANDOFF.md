...
**★2026-06-07 後段 (最新): レジュメ構想 ①+②+④core 実装・E2E 確認済 + ガイド読書 永続化★**:
...
**【★次にやる = レジュメ構想の残りフェーズ】** (詳細 ARCHITECTURE「## レジュメ構想 > ### 実装 (2026-06-07)」):
1. **R4 音声入力** (Web Speech API=`webkitSpeechRecognition`・無料・Chrome・日本語、🎤→編集可能テキスト欄、葵が文字起こしを軽く整える、`types/speech-recognition.d.ts` 型宣言が必須)。
2. **R5 自動リンク** (まとまり=1レジュメ自動オープン。現状は「レジュメにする」で都度。+ 必要時だけ別レジュメ呼び出し=教材横断は後乗せ)。
3. 「ヒントちょうだい」(答えは言わず糸口) / 2 周目 (G-C) で自分のレジュメ改稿 / 図クリップ (後乗せ)。
