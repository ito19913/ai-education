...
**次セッションの主な選択肢**:
- **★N9① ノート中核 縦切り MVP を Plan 化 (推奨・次の本丸)★**: 1教材を葵と読む→能動ゲート→ノートエントリ1件生成・永続化→ノート体系図③表示→出典リンク。必要 = ノートのデータモデル+Supabase永続化(1-Bの延長)/論点認定(最小)/能動ゲートUI/ノート体系図③最小描画。大規模なので Plan 推奨。
- **1-B の仕上げ** (任意): Pro 化して 186MB 本で実機確認 / 残課題 = 親 owner 指定 UX / アップロード中リロード時の「PDF 準備中」/ TUS 失敗再試行 UI
- **学習プラン grill 残り 5 論点** ← N9 で「ノートの後」と決定。まとめノートが固まってから「ノートを育てる計画」として再設計する方が軸ができる
- **段階2** (横断検索 embedding) ← N9 で「ノート中核MVPの後」と決定
