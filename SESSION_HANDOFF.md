...
- **「AI 主導ガイド読書」設計確定 (G-1〜G-7、★未実装★)**: 受け身の生徒に葵が一区切りずつ読み上げ・解説 → Q&A → 「分かった→次へ」。区切り選びは AI 自動+タップ / 修正は言葉+タップ / AI の順序は"下書き提案"で子が直せる・ノートは概念単位なので順序ミスは結果を汚さない / POINT・MEMO は本文後に / 難易度は周回数+リクエストで育つ / **能動ゲートは「馴染んでから関所」= 1 周目は受け身でノート化(open)、2 周目以降の振り返りで説明ゲート→理解済み昇格+正式用語で深化** (N3/N8 と整合)。詳細 ARCHITECTURE「### AI 主導ガイド読書」。

**次セッションの主な選択肢**:
- **AI 主導ガイド読書の実装** ← ★最有力★。設計確定済 (G-1〜G-7)。まず実装プラン → 区切りごと解説ステップを `MaterialReadPane` + `aoki-chat-claude` の上に載せる。未決 = 視覚ハイライト (枠 vs 言葉) / 周回数の保存先 / ノート要約の深化のデータ表現 / 状態機械の持ち方 / 既存「学習を開始する」との統合。
- **既存スキャン本の体系図 再抽出 動線** (wasm 修正前登録の本を救う、再登録なしで)。
