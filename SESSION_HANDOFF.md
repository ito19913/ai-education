...
**⏳ 重要な学び**: ito19 さんの実物本 (英文法解説・真英文法大全) は**自炊/固定レイアウトのスキャン本が多く、文字レイヤーが無いので現状は区切れない** (固着はしない・読書は可能・一覧は出ない)。**本格運用には C-8 (スキャン本の低解像度 vision 区切り) が必須**。

**次セッションの主な選択肢**:
- **C-8 スキャン本のまとまり区切り** ← ★最有力★。実物本の大半がスキャンなので、これが無いと実運用で単元一覧が出ない。低解像度 vision + 目次土台で区切る (段階1-A の vision 目次抽出の延長)。
- **まとまり区切りの磨き** (任意): ページ範囲の重なり等の精度 / C-10 大きすぎ分割提案 / C-11 言葉での区切り修正 / 評価コメントの DB 永続化 (今は session キャッシュのみ)。
