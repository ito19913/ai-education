...
**✅ N9① 中核 縦切り MVP 実装 + E2E 確認済 (2026-06-05、C91)**: 「1教材を葵と読む→能動ゲート(説明させる)→理解済みエントリ生成・永続化→ノート体系図③表示→出典リンクで戻る」の最小ループを実装し、本番 DB で実機確認済 (英語教材 p.7 を葵が vision で読み正しい要約→子の説明で能動ゲート通過→「理解済み」で刻む→ノートホーム表示→リロード後も残存)。`note_entries` migration 適用済 (real モード稼働)。詳細 ARCHITECTURE「### N9① 中核 縦切り MVP 実装」。

**次セッションの主な選択肢**:
- **★N9② (推奨・本構想の続き)★**: 未理解(open)ステータス + 定期振り返り / Issue 統合の完全版 (N8 の「学習概念は全部ノート・open は振り返り」)。
- **N9③ 戻り提案 (N6) + 段階2 横断検索 (embedding)** ← まとめノートの横断統合・出典逆参照を強化。
- **N9④ 学習プラン (Phase 5 解体) を「ノートを育てる計画」として再設計** (プラン残り 5 論点はここ)。
- **まとめノート MVP の磨き込み** (任意): 論点認定の精度 / ゲート判定の厳しさ調整 / 体系図③の状態色分け / 複数科目・複数ノート / 問題集リンク。
