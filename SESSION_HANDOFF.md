...
AI-Education プロジェクトの **セッション間引継ぎドキュメント**。次セッションの最初に必ず読む。最終更新: 2026-05-27 (**プロジェクト前提から大転換**: PHILOSOPHY 全書き換え C58 = **コーチング・ファースト型 学習アプリ** へ + 学習プラン再設計 grill 24 問 39+ 論点確定 (C59 SSoT 同期 + 失敗扱い 6 論点 F1-F6 = C60 SSoT 同期)。**Phase 5 で実装した骨格 (LearningPlan 9 ヶ月 / 全期間 GT 化 / 教材ノード生成 / PlanType 5 種等) は解体・再構築方向**。次は **学習プラン再設計 grill 残り 6 論点** (推奨スタート = 中学生主体性 / PlanType / カリキュラム DB / 系統可視性 / 時間予算 / 統合管制))
...
**残り未確定 6 論点** (次セッション grill 推奨順):
1. **中学生の主体性モデル** ← **次の推奨スタート** (F5「戻り誘導 = 親 OPT-OUT 承認」と直接整合)
2. PlanType 5 種扱い (Phase 5 解体級判断と直結)
3. カリキュラム DB 作成・運用 (つまずき遡及エンジンの基盤)
4. 系統 A/B 子可視性 (軽、確認のみで完結する可能性)
5. 時間予算自動制御 (軽)
6. 2 系統の統合管制エンジン (別チャット案の核、採用?)
...
【今日の状態 (60 commit、プロジェクト前提の大転換 + 失敗扱い grill 完結)】
...
【次セッションで進める論点 — 学習プラン再設計 grill 残り 6 論点】

未確定 6 件、推奨着手順:

| # | 論点 | 性格 |
|---|---|---|
| 1 | **中学生の主体性モデル** ← 推奨スタート | 中 (F5「戻り誘導 = 親 OPT-OUT 承認」と直接整合、主体性論点と密接) |
| 2 | PlanType 5 種の扱い | 中 (廃止 / 再編 / 系統との関係) |
| 3 | カリキュラム DB の作成・運用 | 中 (AI 検索駆動、具体運用、信頼性担保、つまずき遡及エンジンの基盤) |
| 4 | 系統 A/B 子に見せるか | 軽 (ito19 さん「全タスク見せていい」発言、確認のみで完結可能性) |
| 5 | 時間予算自動制御 | 軽 (別チャット案、ito19 さんスルー、保留 or 採用?) |
| 6 | 2 系統の統合管制エンジン | 中 (別チャット案の核、採用?) |

**次セッション開始時のアスク**:
「学習プラン再設計 grill 残り 6 論点のうち、(1) 中学生主体性 から始める? それとも別の論点から?」

推奨: (1) 中学生主体性 = 小学生は親承認モデル (D1) 確定、中学生は未確定 (D2)。失敗扱いの F5「戻り誘導 = 親 OPT-OUT 承認」を中学生にも適用するか、もっと自立的にするか。中学生対象が大きいので、これを決めないと UX が宙に浮く。
