...
2026-05-27 セッション。Phase 6 smoke test 完了後の引継ぎから、ito19 さんが **学習プラン構造をゼロベースで見直す方針** を提示。同セッションで PHILOSOPHY 全廃止 + コーチング・ファースト型への思想転換 (C58) → 続けて、学習プラン構造の grill を 16 問詰めて確定。**Phase 5 で実装した骨格 (LearningPlan 9 ヶ月、全期間 GT 化、教材ノード生成、PlanType 5 種等) は大幅に作り直し** になる。
...
| C2 | ノード設計の大転換 | **教材アップロードから体系図ノードを作る方式を廃止** (Phase 5 解体級) |
| C3 | カリキュラム源 | **小〜高の全カリキュラムを事前に AI が検索して用意** (中高一貫校対応で高校まで) |
...
### 未確定論点 (残り 6 件、次セッション以降で grill)

| # | 論点 | 性格 |
|---|---|---|
| 1 | 中学生の主体性モデル | 中 (小学生 = 親承認確定、中学生は?) |
| 2 | PlanType 5 種の扱い | 中 (廃止 / 再編 / 系統との関係) |
| 3 | カリキュラム DB の作成・運用 | 中 (AI 検索駆動、具体運用、信頼性担保) |
| 4 | 系統 A/B 子に見せるか | 軽 (ito19 さん「全タスク見せていい」発言、確認のみ) |
...
| 主体性 | 完全本人主体 (Q15 一貫) | 小学生 = 親承認、中学生未確定 | 🟡 拡張 |
...
2. **中学生の主体性モデル** grill ← **次の推奨スタート** (失敗扱い F5「戻り誘導 = 親 OPT-OUT 承認」と直接整合)
3. **PlanType 5 種扱い** grill (現行 5 種をどうするか)
