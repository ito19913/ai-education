...
## まとめノート構想 — このアプリの最終ゴール (2026-06-05、★北極星・未 grill★)
...
**体系図が 3 層になる (要整理)**:
- ① カリキュラム体系図 (C2/C3 撤回で「DB は持たない、教材ベース」方針) / ② 各教材の体系図 (段階1-A、目次ベース実装済) / ③ **ノートの体系図 (新、子ども個人の理解地図)**

**★次に grill する詰めどころ (agenda、未確定)★**:
| # | 論点 | 性格 |
|---|---|---|
| N1 | ノートの単位 (単元ごと / 概念ごと / 子の言葉ベース、粒度) | 中 (データモデルの根幹) |
| N2 | 3 層体系図 (カリキュラム / 教材 / ノート) の関係と表示 | 中 |
| N3 | 「理解できた」の判定 = いつノート化するか (ヒアリング合図 / ミニテスト連携 F4) | 中 (コーチング・ファースト整合の核) |
| N4 | 横断の範囲 (科目内のみ / 科目もまたぐ) | 中 |
| N5 | 問題集の問題がノートに入る形 | 中 |
| N6 | ゆいの戻り提案ロジック (ノート体系図のギャップ検出 → 同教材 / 別教材の判定) | 中 |
| N7 | ノート生成の UX (いつ・どう書き込まれるか、子の編集権、AI の下書き度) | 中 |
| N8 | 既存 Issue / 課題 chat / 教材体系図 との関係・統合 | 中 |
| N9 | Phase 5 解体・段階2・本構想 の実装順序 | 中 (ロードマップ) |

**実装は grill 完了後**。本構想は Phase 5 解体プランや段階2 (横断検索) と密接 → grill で順序も整理する。
