...
## まとめノート構想 — このアプリの最終ゴール (2026-06-05、★北極星・grill 確定 N1-N9★)

長い試行錯誤の末、ito19 さんがプロダクトの **最終ゴールを確定**: **子ども自身が AI との対話で作り上げる、教材横断の「オリジナルまとめノート」**。コーチング (ゆい) もティーチング (葵) も、最終的にはこの 1 冊のノートを育てるために働く。PHILOSOPHY「章 6: このアプリの最終ゴール」と対 (思想面はそちら、本セクションは設計面)。

**思想的な軸 (PHILOSOPHY 創設の why と一致)**: 会計士試験で得た「暗記でなく、頭の中にツールセットを組み立てて使う」を **見える形に外部化したもの = まとめノート**。AI が説明して「分かった気」になったものはノートにしない。**ゆいのヒアリング (F4) で本当に理解できたものだけ**が刻まれる = 「分かった気」を排除する関所。

**ビジョン (ito19 さん 2026-06-05 発話、確定方向)**:
1. **作り方**: テキストを葵と一緒に読み (1-C 読書ビュー)、内容を**子の言葉で要約**してノートに落とす。ヒアリングで理解確認 → ノート化
2. **教材横断**: 一冊に閉じず、複数テキスト + 問題集が 1 つのノートに統合。「これさえ見れば全部分かる」を目指す
3. **出典逆参照 (citation)**: 各要約エントリ → 元テキストの何ページから来たかを辿れる (= 段階1 のページ単位 + 段階2 の embedding 横断検索が土台)
4. **問題集の取り込み**: 問題集の問題もノートに入る
5. **ノートの体系図**: ノート自身から立ち上がる「自分だけの体系図」。復習はノート + この体系図を眺めて行う。インプットはノート、問題集と行き来
6. **ゆいの戻り提案**: ノートの体系図の薄い/つながっていない箇所を検出 → 「ここに戻ろう」、体系図 (同教材) で足りなければ「別テキストで補おう」と**コーチ (ゆい) が提案**

**既存資産との接続 (今日までの実装がそのまま土台)**:
- 1-C 読書ビュー = テキストを葵と読む入口 / 1-B 永続化 = 教材・体系図・PDF が残る / 段階2 横断検索 (embedding) = 出典逆参照と横断統合のエンジン

### grill 確定 (2026-06-05、N1-N9)

> 注: 下の「ビジョン」項1「子の言葉で要約」は N3 で **「中身は AI 要約 (正しい内容)、刻む前に能動ゲート」** に更新。項6 の戻り提案は N6 で具体化。以下が確定版。

| # | 論点 | 確定 |
|---|---|---|
| **N1** | ノートの単位 | **概念 (知識ノード) 単位**。1 エントリ = 概念 + AI 要約 + 出典 + 関連問題リンク + 自分メモ + 理解ステータス。概念をアンカーにする (中身は AI 要約) |
| **N2** | 体系図の整理 | **子が見る地図はノート体系図③ 1 枚だけ (ホーム)**。教材は目次リストのみ (マップ廃止、データは裏に保持)。統合ビューは画面化しない。「全教材②の合計 − ノート③」の差分は**内部信号**として持ち、ゆいが対話で小出し (壁にしない)。カリキュラム①は DB で持たず全教材②の合計で近似 |
| **N3** | 理解の関所 | ノート中身 = **AI 要約 (正しい内容、子の言葉ベースにしない)**。刻む前に**能動ゲート** (子に説明させる or ミニ確認)、受け身の「はい」では刻まない。AI が「こう整理した、こう理解だね」と返して確認。ゲートは「ノートに入れるか」でなく「**理解済みに昇格させるか**」を決める (N8) |
| **N4** | 横断の範囲 | **科目は絶対またがない**。科目内は複数ノート可だが**追加は常にメインへ自動**、分割/結合は後からの整理。MVP は実質 1 冊スタート (構造だけ複数対応) |
| **N5** | 問題集 | 問題はノートに入れず**概念エントリからリンクで飛ばす**。**論点認定 (学びの断片→どの概念か を AI が判定) がエンジンの土台機能** (テキスト記述・問題・つまずき すべてを論点に認定)。テキスト=インプット源/問題集=確認源 |
| **N6** | 戻り提案 | **2 段階**: ①まず手持ちの前提へ戻る (最大2学年、F6) → ②足りなければ別テキスト推奨 (G3)。**前提関係は保存せず AI が都度判断** (カリキュラム DB なし方針と整合)。承認は F5 親 OPT-OUT、ゆいが対話で小出し |
| **N7** | 編集権・動線 | **本体 (AI 要約) は書き換えさせず正しさを守る** (直すなら AI 経由)。子は**自分メモ追記・削除/非表示**で所有。ゆいメニューに「ノート」主動線、ホーム = ノート体系図③ → ノードクリックでエントリ (AI 要約 + 出典リンク + 関連問題リンク + 自分メモ) |
| **N8** | Issue 統合 | **学習した概念は全部ノートに入る** (包括的)。エントリは**理解済み / 未理解 (=Issue) のステータス**。Issue は別リストでなく**ノート内の open エントリ**。open が残る限り**定期振り返り**、能動ゲート通過で理解済みに昇格。ケアレス (F6) は概念エントリにせず記録のみ。**触れた概念は地図に出すが未踏の山は出さない** |
| **N9** | 実装順序 | **①ノート中核 縦切り MVP (段階1 ベース・embedding 不要) → ②Issue 統合 + 定期振り返り → ③戻り提案 + 段階2 横断検索 (embedding) → ④学習プラン (Phase 5 解体) を「ノートを育てる計画」として再設計・接続**。プランも段階2 もノートの後。**北極星確定でプランの目的が「まとめノートを効率よく育てること」に再定義** |
...
### N9② 未理解(open) + 定期振り返り 実装 (2026-06-05、✅ 実装、grill Q1-Q5)

N8「学習概念は全部ノート・理解済み/未理解(=Issue)ステータス・未理解は定期振り返り・ゲート通過で昇格」を実装。**既存 Issue システム(mock・未永続化)は触らず note_entries の上に加算式**(Q1)。新規テーブル/repo なし(status 列は N9① 既存)。
...
**問題**: 「AI が要約を先に作る→子が理解」だと AI のノートを子が理解しただけ。**対話と子の意思を入れて子の 1 冊にする** (ito19 さん指摘)。データモデル不変。

**確定フロー (grill Q1-Q4)**:
- 読書ビューに **「▶ 学習を開始する」**(葵が今のページを自分から説明) + **「ノートにまとめる」は常時押せる** (いつまとめるかは子の判断、Q1)
- まとめ要約に **葵 chat の対話を反映** (本人の質問・腑に落ちた点が入る=オリジナル要約 / 対話なし=従来のページ要約、Q1/Q2)
- まとめ中に **「メモしたいことある?」を聞き取り** → 自分メモ欄(user_note) へ (本体=正しい要約は守る、Q2)
- まとめ時の **説明ゲートは任意**: 説明通過→理解済み / スキップ→open(=Issue) (関所維持、Q3)
- **1 回まとめる = 1 概念エントリ** (細分化は対話で自然に、自動分割しない、Q4)

**実装**: `summarizeConceptForNote` が dialogue を受け取る / `NoteGateDialog` に review ステージ(要約確認+メモ取り込み+ゲート任意化) / `MaterialReadPane` に「学習を開始する」(respondViaAokiChat キックオフ、葵の説明だけ積む) / `insertNoteEntry` が userNote 保存。

**+ 装飾 (C95)**: 読書ビューの葵 chat に先生アバター(SubjectTeacherAvatar) + 吹き出し + MarkdownText 描画 + タイピングインジケータ。

---

### まとめノートの「まとまり (一単元) 区切り」設計 (2026-06-06、★grill 確定 M1-M10、✅ デジタル本 + スキャン本 (C-8) で実装・E2E 確認済★)
...
**確定 (grill M1-M10)**:
- **M1 まとまり = 1 概念** (目次の章より細かく、話が切り替わる所で区切る)。**1 まとまり = 1 ノートエントリ** (C94「1まとめ=1概念」/ N1 と整合)
- **M2 範囲は AI が中身を読んで確定**。子はドラッグも数値も触らない。修正したい時だけ葵に**言葉で**粗く (「もっと短く」「次の所まで続いてる気がする」→ AI が区切り直す)
- **M3 (Y) AI は中身ベースで PDF の紙番号 (PDF page index) で直接区切る**。目次/印刷ページ番号はヒントに格下げ。**印刷⇄PDF のズレ問題は原理的に消滅**、人間はページ数に一切関与しない。← 印刷ページ番号は元々「目次との整合性」のためだけに存在した内部数値
- **M4 全書プリ区切りを登録時にバックグラウンドで実行** (既存の目次 vision 処理 = 段階1-A/1-B に乗せる、完了通知)。**スケジュール (N9④) は全単元の並び+ボリュームが分からないと配分できない**ので全書区切りが必須 → 登録時に済ませる。コストは PDF 種別 2 段階:
  - **デジタル PDF (文字レイヤーあり = 市販テキスト大半)**: 本文テキスト全抽出 → AI がテキストで区切る。**vision 不要・激安・速い**
  - **自炊スキャン PDF (文字レイヤー無し = 186MB 本等)**: vision 必須で重い。**低解像度 + 目次の区切りを土台に境界付近だけ確認**して精密化 (全ページ高解像度 vision はしない)。重いがバックグラウンド + 完了通知で体感無痛
  - 区切り結果 (概念名 + PDF 紙番号範囲) を material に保存 → 以後スライダー・精密区切り・N9④スケジュールが**即時**
- **M5 左の縦スライダー = 全ページサムネ (遅延描画) + 概念境界オーバーレイ + 今やってる範囲の色帯 + ノート化済み緑チェック**。「本全体のどこ・どこまでやったか」が一目。**編集不可の"見る地図"専用** (M2)
- **M6 セッションは 2 フェーズ**: ①AI オリエン (「ここはこういう所、ここに注意。まず通して読もう、分からない所は飛ばして OK」) → 範囲を**通読** (まだ刻まない) → ②**概念ごとに**深掘り + 能動ゲート + まとめ。スライダーで「今どの概念をまとめてる/どこまで刻んだ」が見える
- **M7 範囲提示の起点**: 教材を開いて学習開始すると AI がノート進捗を見て**次の未まとめ概念の範囲**を提示 / 子が任意ページに飛んでいればそのページを含む単元を提示 (`findConceptForPage` を PDF-index ベースに更新して流用)
- **M8 要約は範囲全体の vision + 通読中の対話を反映**して作る (概念単位なので軽い、`summarizeConceptForNote` の dialogue を活用)。範囲が大きすぎる時 (目安 ~10ページ超) は葵が「2 つに分ける?」提案
- **M9 「今日の範囲」は連続する複数まとまり可** (例: 時制の章 = 3 まとまり) だが**ノートは概念ごとに別々に刻まれる**
- **M10 N9④への波及**: まとまり (概念) = スケジュール配分の最小単位。全書区切りがあるのでテキスト + 期日から AI 推奨スケジュールが作れる (本格 grill は N9④)

**未決 (実装/プラン時)**:
- 既存の体系図②(目次ベース・印刷番号 `AiExtractedNode.pageRange`) と新しい中身ベース概念セグメント (PDF-index) の**関係** — 置き換え? 目次は親階層として残す? 体系図③(ノート地図) との繋ぎ
- セグメントの**データ構造** (material への保存形 / note_entry からの参照) + マイグレーション要否
- スキャン本全書区切りの**具体コスト見積もり** (186MB 本で現実的か、低解像度の塩梅)
- 「学習を開始する」ボタンの**再構成** (今=今ページ説明 → 新=単元提示 + オリエン + 2 フェーズ誘導)
...
#### 実機 E2E 確認 + 高速化 + バグ修正 (2026-06-06、✅ デジタル本で完成)

MVP 実装後、本番 DB で実機検証しながら C-9 縦スライダー・単元メニュー入口を前倒し実装し、
発見したバグを潰してデジタル本で完成させた。**migration 2 本は本番 DB に適用済 (real モード稼働)**。

実装/改善:
- **C-9 縦スライダー (M5)**: `components/materials/PageThumbnailRail.tsx` 新規。全ページサムネを
  IntersectionObserver で遅延描画 + 今のまとまりを色ハイライト + まとまり先頭に概念名ラベル +
  ノート化済み緑チェック + 現在ページ sky リング。クリックでジャンプ (見る地図、編集不可 M2)。
- **入口 = まとまり一覧から選ぶ (M6/M7)**: 開いた直後に葵 chat に**全まとまり (前付け除く・ページ順) の
  一覧**を提示 → 子が選ぶと `startUnit` でその単元先頭へジャンプ + オリエン (範囲提示+通読促し+軽い確認)。
  「まとまりを選ぶ」ボタンで再表示。`showUnitMenu` state。
- **オンデマンド生成 + 永続化**: 読書ビューを開いた時に DB 区切りが無ければ葵がその場で生成 → real なら
  `updateMaterialSegments` で DB 保存 (既存教材も再登録なしで永続化、次回は即一覧)。
  `sessionSegmentCache` + `attemptedSegmentation` (module Set) で 1 教材 1 回。
- **高速化**: 区切りモデルを Opus 4.8 → **`claude-haiku-4-5`** (構造抽出は Haiku で十分・大幅高速)。
  評価コメント `generateMaterialReviewViaClaude` を `sessionReviewCache` で 1 教材 1 回に (Opus が
  毎回 12〜43 秒走って API 占有していた主因を解消)。
- **非ブロッキング化**: 区切り生成中も読書ビュー + 「学習を開始する」(今ページ) が使える。全画面スピナー廃止。

実機検証で潰したバグ (3 連):
1. **前付け混入**: 表紙・目次・使い方が 1 まとまりとして出ていた → segment プロンプト強化 (前付け・索引等は
   出さない、最初は本文最初の概念から) + `isFrontMatterName` で学習開始時もスキップ。
2. **無限ループ (スキャン本)**: 区切り 0 件で終わる教材が再レンダ毎に再生成し続け「ずっと準備中」→
   `attemptedSegmentation` で 1 回だけ試す。
3. **★自己キャンセル固着 (本丸)★**: 生成エフェクトの依存に `segmenting` を入れていたため
   `setSegmenting(true)` で自エフェクトが再実行→ cleanup が in-flight を cancel→ `setSegmenting(false)` が
   走らず `true` で固着 (デジタル本でも reopen で「ずっと準備中」)。→ 依存を
   `[loaded, material.id, material.conceptSegments]` に限定 + cancel 機構撤去 (投げっぱなし+finally で必ず false)。
   StrictMode 二重実行でも固着しない。

**✅ E2E 確認済 (デジタル本「基本マスター BASIC 英文法」)**: 開く→数秒で単元一覧 (前付け除外・概念単位・
ページ範囲付き)→単元選択でジャンプ+オリエン→ノートにまとめる→リロードで即一覧 (DB 永続)。
**⏳ スキャン本 (英文法解説・真英文法大全等) は文字レイヤー無しで区切れず一覧は出ない (固着はしない、読書は可能)
→ C-8 (低解像度 vision 区切り) が本格運用に必須**。ito19 さんの実物は自炊/固定レイアウトのスキャン本が多い。

追加実装ファイル: `components/materials/PageThumbnailRail.tsx` (新規) / 改修 `components/materials/MaterialDetailView.tsx`
(評価コメントキャッシュ)。区切りモデル `lib/admin/segment-claude.ts` = haiku-4-5。

**次の選択肢**: ① **C-8 スキャン本の区切り** (実物本の大半がスキャンなので最有力) / ② 区切り精度の磨き
(ページ範囲の重なり等、軽微) / ③ 評価コメントの DB 永続化 (今はセッションキャッシュのみ) / ④ N9③ (戻り提案+
段階2 embedding) / ⑤ N9④ (プラン再設計、まとまりが土台)。

#### C-8 スキャン本のまとまり区切り 実装 + JBIG2 wasm 描画修正 (2026-06-06、✅ 実物スキャン本で E2E 確認)

ito19 さんの実教材「税法実務講座 税法入門 法人税 (TAC)」(182p 自炊スキャン) で「まとまりが出ない」を
再現 → C-8 を実装。実装中に **JBIG2 描画バグ** も発見・解決した (別の根本問題)。

**(0) JBIG2 wasm 描画修正 (commit `7b1dc4c`)** ★スキャン本全般に効く独立の修正★:
- 真因: pdfjs-dist 6 は JBIG2/JPEG2000 デコーダを wasm に分離。wasm 未配置 + `wasmUrl` 未設定だと
  **JBIG2 圧縮の自炊スキャン本の本文ページが真っ白**になる (表紙は JPEG なので出る = 「テキストは真っ白」の正体)。
- 解: `node_modules/pdfjs-dist/wasm/*.wasm` を `public/pdfjs-wasm/` に配置 + 3 つの `getDocument` に
  `wasmUrl: "/pdfjs-wasm/"` + `proxy.ts` の matcher 除外に `.wasm` 追加 (worker の wasm fetch が認証で
  弾かれないため、mjs/js と同理由)。+ `renderPageToJpegAt(doc,page,longEdge,quality)` 追加 (低解像度描画)。

**(1) C-8 まとまり区切り (commit `699f8cd`)**: 2 経路 (grill 確定はセッション内、要旨は上記 M4 のスキャン段):
- **ハイブリッド経路 (目次あり、主経路)**: C85 目次 vision で抽出済みの体系図ノード (印刷ページ付き) を土台に
  **Opus 4.8 が「1 概念」へ再グルーピング** (`regroupTocIntoSegments`、①②に割れた同一テーマだけ束ね、別テーマは
  束ねない。ページ番号は出させず tempId だけ → 範囲はノードから決定的に算出)。印刷→PDF 紙番号の
  **オフセットを 2 点較正** (`readPrintedPageNumbers`=ノンブルを Opus vision で読む)、不一致なら
  まとまりごと個別探索にフォールバック。`source:"outline"`。
- **全ページ vision 経路 (目次なし or A 失敗)**: 低解像度ページ画像を順に Haiku で読み **PDF 紙番号で直接区切る**
  (`segmentScanByVision`、チャンク分割+隣接同名マージ)。`source:"scan-vision"`。
- 配線: `MaterialReadPane` のオンデマンド生成エフェクトの `hasTextLayer:false` 分岐で `buildScanSegments` を
  呼ぶ (デジタルの `segment-claude` と同じ cache/state/DB 永続機構を共通化)。
- 実装ファイル: `lib/admin/segment-scan-claude.ts` (3 Server Actions) / `lib/admin/scan-segment-builder.ts`
  (client オーケストレータ) / `lib/admin/pdf-extract-text.ts` (`renderPageToJpegAt`) / `MaterialReadPane.tsx`。
- 粒度モデル: regroup は当初 Haiku → 別テーマを過剰マージし「荒すぎ」(7-8 まとまり)。**Opus 4.8 + 「迷ったら
  束ねず分ける」プロンプト**に変更 → 本に忠実な 26 まとまりへ改善 (ito19 さん確認)。

**✅ E2E 確認 (TAC 法人税 182p スキャン)**: 第1〜4章 全体を 26 まとまり (交際費・寄附金・減価償却・留保金課税…
本の言葉に忠実) に区切り、選択でジャンプ・本文表示 (wasm 修正後) が成立。ito19 さん「完璧」確認。

**★Lesson (重要)★**: 体系図 (`extractedNodes`) が **wasm 修正前に抽出された本は不完全** (抽出時に目次ページが
...
**ito19 さんのビジョン**: テキストを主体的・能動的に「読ませる」のは良くない。AI の生徒は**比較的受け身**でよい。
→ **葵がテキストを一区切りずつ読み上げ・解説し、その都度 Q&A、「分かった→次へ」で進む**ガイド読書。
「できる子の"どこで区切るか・どの順で読むか"の力を AI が代行する」= このアプリの核心価値の具体化。
現状の「ページ/まとまり全体を一気に説明 + 自由チャット」より**細かい歩幅で AI が手を引く**。

**grill 確定 (2026-06-06)**:
| # | 論点 | 確定 |
|---|---|---|
| G-1 | 次に解説する「読んで欲しいとこ」の選び方 | **基本 AI 自動 + 子がタップで飛べる** (受け身方針。子が「どこ読む?」と能動的に選ぶのは避ける) |
| G-2 | 「これが違う/ここを読みたい」修正手段 | **言葉 + タップ**。葵はブロックを vision 認識済みなので「POINT のとこ」「右の判定表」で切替可 / タップ→近辺ブロックを解説。精密な枠描き UI は不要 |
| G-3 | AI の区切り・順序が外れる問題への姿勢 | **AI の区切り・順序は"正解"でなく"下書きの提案"。子がいつでも直せる。最終ノートは概念 (まとまり) 単位なので、道中の順序ミスはノートの中身を汚さない** = 「AI を完璧にする」でなく「AI は外さない前提で作らない」 |
| G-4 | POINT / MEMO 等の補足ブロック | **本文とは別、本文を一通り流した後に差し込む** (プロンプト規約。葵は vision で POINT/MEMO/補足を種類判定できると実証済み) |
| G-5 | ページまたぎ・左右往復 | まとまりの全ページを葵に渡し**内容ベースで繋ぐ**。弱点なので「片ページ本文→対向ページ→補足」の無難な既定順 + 迷えば葵が「次は左の続き? 右の表?」と軽く聞く (子は受け身のまま選ぶだけ) |
| G-6 | 難易度レベルの駆動 | **周回数 + その場リクエスト**。初回はやさしく、2 周目以降は正式用語で深める (周回数を記録)。「もっと簡単に/詳しく」はいつでも即反映。ノート要約も周回で深まる |
| G-7 | 能動ゲート (N3、"分かった気"排除) の扱い | **「最初から関所」でなく「馴染んでから関所」**: 1 周目は受け身で通読・解説してノート化 (open のまま可)。**2 周目以降の振り返りで「自分の言葉で説明して」(能動ゲート) → 通れば理解済みに昇格 + ノートを正式用語で深める**。受け身の入口 + N3 維持 + 周回数モデル + N8 (open→理解済み昇格) が全て整合 |

**AI 能力の実測 (留保金課税ページで検証、設計の前提)**: 葵 (Opus vision) は見開きの全ブロックを**種類付き**
(本文/見出し/表/図/POINT/MEMO/補足) + 概略位置で検出でき、**POINT/MEMO を「後で読む補足」と正しく判定**、
教える順序も提案できた。一方**読む順序 (左右往復・ページまたぎ・"例外の例外") は自分で「迷う」と告白** =
ブロック認識は信頼できるが順序は外しうる → G-3/G-5 の「提案 + 子が直せる + ノートは概念単位」設計の根拠。

**未決 (実装/プラン時)**: 「読んで欲しいとこ」の視覚ハイライト (葵 vision のおおよそ座標で枠を描く vs 言葉だけ) /
タップ座標→ブロック特定の精度 / 周回数の保存先 (note_entry に level/回数列?) / ノート要約の「更新 (深化)」の
データ表現 (同一エントリ更新 vs 履歴) / ガイド読書の状態機械 (現在ブロック・進捗) の持ち方 / 既存「学習を開始する」
+ 自由チャットとの統合 (置換 or 併存)。

**実装の起点 (現状コード)**: `MaterialReadPane.tsx` (`runOrientation`/`handleSend`/`openNoteGate`) /
...
- **青枠を直接ドラッグで手動調整** (`EditableHighlight` in `MaterialReadPane.tsx`): 枠本体ドラッグで移動 / 四隅ハンドルで拡大・縮小。
- 読み方の方針 (ito19 さん): 塊を選ぶ → そこをベースに質問・不明点を聞く、を一区切りずつ。
- bbox 精度自体の改善は不要 (手動で直せるため棚上げ)。
- **★2026-06-08 修正: 青枠を別ブロックへ大きく動かしたら「選択し直し」**: ドラッグは従来「今の
  ブロックの枠の微調整」だけで、別ブロックへ動かしても `guidedIndex` が変わらず「ここを解説」が
  前のブロックを読む不具合があった (ito19 実機指摘)。pointerUp 時に、動かした枠の中心が別ブロック
  (同ページ) の bbox 内に入っていれば、そのブロックを選択し直す (元ブロックの枠は pre-drag 位置へ
  戻す、保存しない)。中心が他ブロックに入らない小移動は従来どおり「微調整」として保存。
  `EditableHighlight` に `onDragStart` (元 bbox 通知) + `preDragBboxRef` + `commitGuidedBbox` で実装。
- **★2026-06-08 さらに見開きで「ページまたぎ移動」対応** (ito19 実機「ページをまたいで移動できない」):
  選択し直しの判定を bbox 中心ではなく **指を離した画面座標 (dropClientX/Y)** に変更。`EditableHighlight`
  の `onCommit(dropX, dropY)` がドロップ座標を渡し、`commitGuidedBbox` が**見えている各ページ canvas の
  矩形でヒットテスト**→ドロップ先ページ内の点を含む最小ブロック (無ければ中心最近傍) を選択。これで
  見開きの反対ページへ枠をドラッグして離すと、そのページのブロックに選択が移る (枠はページ端で一旦
  止まるが離した先で判定)。同ページの選択し直し・微調整も同じ経路で動く。

#### 2026-06-07: ガイドプラン + 青枠調整を DB 永続化 (commit `020bee2`、✅ migration 本番適用済)

**なぜ**: ガイドプラン (`GuidedBlock[]`) は概念を開くたび Opus vision で作り直し、session メモリにしか無かった (リロードで消える・非決定的)。block ID は生成順依存なので、作り直すと**子が手で動かした青枠 (bbox) が別ブロックに当たり**、手動調整値を保存しても意味を持たなかった。→ grill で「**プランごと固定保存**して初めて調整値が意味を持つ」と確定。

**何を/どう動くか**:
- migration `20260607010000_add_guided_plans.sql`: `materials.guided_plans` JSONB (default `{}`) を追加。形は `{segmentId: GuidedBlock[]}`。RLS は既存ポリシーが効く。
...
## レジュメ構想 — まとめノート構想の再定義 (2026-06-07、★grill 確定・未実装★)

ito19 さんが北極星「まとめノート構想」を一段鋭くした。**「ノート」を「レジュメ」に改名し、作る人を AI から子へ移す**。

**why (PHILOSOPHY「章 6」と対、思想面はそちら)**: 小中高の「ノート」は**書くだけの作業**になりがちで、書いた事で勉強した気になる罠がある。対して**「レジュメ＝要約」は、それを見れば全体を思い出せる**事がゴール。あえて子に使い慣れない言葉を選ぶ事で「ノートとは別物を作る」合図にする。会計士試験で得た「頭の中のツールセットの外部化」= レジュメ。自分で書いた要約でないと記憶のトリガーにならない、という洞察が核。

### grill 確定 (2026-06-07、R1-R8)

| # | 論点 | 確定 |
|---|---|---|
| **R1** | 呼び名 | 成果物=**レジュメ** / 動作=**レジュメにする** / 地図=**レジュメ体系図** / 一覧画面=**レジュメ** / 未理解=**Issue 据え置き** (大人語にあえて先に慣れさせる)。**範囲 A = 子に見える文言のみ**改名、内部コード/DB 名 (`NoteEntry`/`note_entries`/`lib/notes`) は据え置き (migration リスク回避) |
| **R2** | レジュメ本文を書く人 | **子が書く (b 案)**。N3 を更新: これまで「捨てていた」子の "自分の言葉の説明" を**レジュメ本文に昇格**。葵は**添削だけ**。能動ゲートとレジュメ作成が 1 つの行為に統合 |
| **R3** | お手本を見せるか | **見せない**。AI の正しい要約は**添削の裏でだけ**使い、お手本としては出さない (写経=作業への逆戻りを防ぐ核心)。**教材そのものは横に見えて OK** (自分の教科書、参照は自然)。写経防止は「3 色添削＋自分の言葉縛り」で担保 |
| **R4** | 入力方式 | **音声メイン＋テキスト併用**。「説明する=しゃべる行為」= 最強の想起練習 (ファインマン)。**Web Speech API** (`webkitSpeechRecognition`、無料・Chrome 内蔵・日本語可) で実装。文字化→**編集できるテキスト欄**に入る。葵が文字起こしを**軽く整えるだけ** (フィラー/誤認識のみ、内容は足さない/変えない)。詰まったら「ヒントちょうだい」で小出し。精度不足なら後で Whisper に差替 (MVP では過剰) |
| **R5** | レジュメ↔教材リンク | **学習しながら自動** (事前手動はナシ)。まとまりを学習して「レジュメにする」→ **その概念のレジュメが自動オープン** (無ければ新規/あれば続き)。`sourceSegmentId`/`sourceMaterialId` が土台。**「別のレジュメに入れたい」時だけ呼び出して上書き** = 教材横断で 1 レジュメに集約 (北極星)、応用機能として後乗せ |
| **R6** | レイアウト | レジュメモード時 **［サムネ(任意)｜教材・単一ページ｜レジュメ書く pane｜葵 chat］の横並び** (実機で上下より横並びが良いと確定)。教材の右にレジュメ書き欄、さらに右に葵 chat。**葵 chat は残す** (書きながら質問・「ここを解説」で教われる)。**教材は単一ページ強制 (見開き禁止)**。各境界はドラッグ可変 |
| **R7** | 添削方式 | **3 色フィードバック**: ◎合ってる(褒める) / △抜け(「○○に触れると完璧」=**何を**は言うが答えは書かない) / ✕違う(指摘のみ)。子が自分で直して再提出 (🎤 or テキスト編集) を数回ループ。**葵は答えを書かず方向だけ示す** |
| **R8** | 理解済み/中断 | **理解済み = ✕(明確な誤り)ゼロ ＆ △(重大な抜け)ゼロ**。いつでも「今日はここまで」で中断 OK → 書いた分は **Issue (未理解) として保存**、後で振り返りで続き。N8 の open エントリと整合 |

**未決 (実装時 or 別 grill)**: レジュメ本文の保存先 (`aiSummary` フィールド流用 or 新カラム) / 「軽く整える」と「3 色添削」の AI 呼び出しを 1 本化するか分けるか / 図・教材スニペットのレジュメ挿入 (図解クリップ、**今回スコープ外で後乗せ**) / 2 周目 (G-C) で自分のレジュメを改稿する流れと既存 G-C の統合 / 通読フェーズ→レジュメフェーズの遷移トリガー UI。

**既存資産との接続**: NoteGateDialog (能動ゲート・要約生成・メモ取り込み)・note_entries (status/user_note/sourceSegmentId)・ConceptSegment (まとまり=1レジュメ)・ガイド読書 (通読フェーズ)・読書ビュー 3 ペイン (R6 の土台) がそのまま再利用できる。R2/R3 で **summarizeConceptForNote (AI が要約) → 子が書く + judge を 3 色添削に作り替え**、R4 で音声入力新設、R6 で chat 枠をレジュメ pane に。

**実装順 (案、Plan で詰める)**: ①呼び名リネーム (範囲 A) → ②レジュメ pane＋教材単一ページ切替 (R6) → ③音声入力 (R4) → ④子が書く＋3 色添削フロー (R2/R3/R7/R8) → ⑤自動リンク (R5)。

### 実装 (2026-06-07、✅ ①+②+④core を実機 E2E 確認済、commit `7e07fd2`〜`da0f7e3`)

- **フェーズ① 呼び名リネーム (`7e07fd2`)**: 子に見える文言を ノート→レジュメ / 「ノートにまとめる」→「レジュメにする」 / 「ノート体系図」→「レジュメ体系図」 / ハブのチップ「ノート」→「レジュメ」。**★落とし穴対策**: `tutor-mock.ts` の NL 分類器に「レジュメ」系トリガを追加 (旧「ノート」系は後方互換で残置)+右ペイン応答文をレジュメ表記に。チップ文言と分類器を 1 コミットで同期し右ペインが開かなくなる回帰を防止。内部コード/型/DB/ルートキーは据え置き (範囲 A)。
- **フェーズ②+④core 横並びレジュメ pane (`f7ec99e`/`3d32203`)**: 新 `components/notes/ResumePane.tsx` (教材の横でレジュメを書く常設ペイン) + 新 `lib/notes/note-gate-claude.ts > reviewResume()` (教材ページ画像を vision で直接読み、子のレジュメ本文を ◎/△/✕ の 3 色で添削、答えは書かない、wrong あれば resolved=false、Opus 4.8、mock 付き)。`MaterialReadPane` に `resumeMode` state + `openResume()` (旧モーダル NoteGateDialog の create 描画を撤去し ResumePane 起動・まとまり先頭へジャンプ)。横並び ［サムネ｜教材単一｜レジュメ書く｜葵 chat］、教材は単一ページ強制。**ガイド操作 (前/次・ここを解説・やさしく・詳しく) はレジュメモード中も維持** (書きながら葵に教われる、ito19 要望)。レジュメ本文は `note_entries.ai_summary` カラムに格納 (migration 不要、内部名据え置き)。
- **誤マッチ修正 (`5418b04`)**: `ConceptSegment.id` は教材内ユニーク (seg-1 等) のため、既存レジュメ検索と緑チェック (`notedSegmentIds`) を `sourceMaterialId` で必ず絞る (別教材の同名 seg が混ざり英語レジュメが法人税に出た実機バグ)。
- **確定フロー 3 アクション (`da0f7e3`、R7/R8 + ito19 確定)**: まとまりの中は自由に書き、**任意で「葵に見てもらう」(途中チェック)**。**「このまとまりを仕上げる」= 関所 (必ず葵の添削を通す、スキップ不可)**。仕上げ→誤りゼロ&重大な抜けゼロなら本人が「理解済みで確定」、残れば「直す/今は Issue で残す」。**「今日はここまで」= 中断 (添削なし→open 保存)**。★「税語」= 法人税の先生「ぜいこ先生」の聞き間違い、意味は葵の添削。書くのは子・AI はチェックのみ ((a) 確定)。
- **✅ R4 音声入力 実装済 (`b2442f3`)**: `types/speech-recognition.d.ts` (Web Speech API 型宣言) + `hooks/use-speech-recognition.ts` (ja-JP/continuous/interim、確定断片を onFinal) + ResumePane に「🎤 話す/停止」(本文へ追記・live 表示・非対応はテキストのみ)。ito19 実機「いい感じ」。
- **★未実装 (次)**: R10 レジュメ(冊)管理 (下記) / R5 別冊への呼び出し / 「ヒントちょうだい」/ 2 周目 (G-C) で自分のレジュメ改稿 / 文字起こしを葵が整える「整える」。

### レジュメ(冊)管理 (R10、2026-06-07 grill 確定・★段階実装 (Phase1 未着手)★)

ito19 さん (会計士) の実体験から: **1 科目 = 1 冊が原則**。バラバラに見えるものが最終的に
1 つの体系として理解できるよう 1 冊に統合される (その中の体系図がレジュメ)。ただし科目に
よっては**例外的に分割** (例: 相続税の「財産評価」と「財産の計算」は別冊が自然)。

**確定仕様**:
- **原則 1 科目 1 冊**。科目を作ると**デフォルトの 1 冊が自動作成**され、まとまり (ピース) が
  そこに溜まって体系になる。
- **2 冊目以降を追加できる** (分野で分ける) / **コピーできる** / **削除できる**。
- **★デフォルト冊は削除不可**。科目に常に最低 1 冊 + デフォルト 1 つを保証。デフォルトを消す
  時は**別の冊をデフォルトに変更してから**消す。
- まとまり→「レジュメにする」は**デフォルト冊に自動**で付く。**別冊に入れたい時だけ呼び出して
  選択** (R5、教材横断もこの仕組み)。

**データ (要 migration)**: 新 `resumes` テーブル (id / owner_id / subject_id / name /
is_default / created_at / deleted_at) + `note_entries.resume_id`。既存ピース (resume_id null)
は科目のデフォルト冊へ移行 (起動時 ensure-default + backfill)。**★現状の穴**: NotesHomeView は
全科目のピースを混ぜて表示している (科目で閉じていない、N4 違反) → Phase1 で 冊/科目 スコープに。

**段階実装 (ito19 「段階的」選択)**:
- **Phase 1**: 科目にデフォルト 1 冊自動作成 + レジュメ体系図を冊ごとに閉じる (科目スコープ) +
  既存ピース移行 + ResumePane が resume_id を書く + 冊名表示 (「○○レジュメ」)。
...
レジュメ構想 R4 で確定済みだった未実装機能。詰まった子に**答えは言わず糸口だけ小出し**。
grill 確定: **押すごとに段階的に濃く** (回数を葵に渡して調節、★答えは最後まで言わない★)。

- **`note-gate-claude.ts > getResumeHint`**: 教材ページ (vision) + 概念 + 今書いた本文 + `hintLevel` +
  既出ヒント を渡し、糸口を 1 つ短く返す (Opus 4.8、max_tokens 300)。level 1=方向/問いかけ →
  2=少し具体 → 3+=かなり近づくが答えの文は書かない。previousHints で繰り返し回避。
- **`ResumePane`**: 「💡 ヒント」ボタン (話す/音声の隣)。押すたび `hints.length+1` を level に渡し、
  返りを水色ボックスに番号付きで積み上げ。まとまり切替でリセット。mock フォールバックは 3 段階固定文。
- 全 tsc/lint/build クリア。**ito19 実機確認済。** これで R4 (音声 + ヒント) が完全実装。

### レジュメ構造化 R11 — 見出しの完全統一 + 宣言 (2026-06-11 grill 確定 R11-1〜R11-8、★未実装★)

レジュメ＝「頭の中の体系図」の本丸。ito19 さんの会計士受験の実体験
(商法・財務諸表論・監査論・経営学の 4 冊で機能した型) を製品に焼き込む。

**前提の三段論法**: 知識を使いこなす ← 知識が頭の中で整理整頓されている ← 必要な時に
レジュメが頭にパッと浮かぶ。その整理を作ったのが次の 2 本柱:

1. **見出しの完全統一** — Ⅰ (ローマ数字) → 1 (アラビア数字) → (1) → ①②③ → ・(箇条書き)
   を全レジュメで完全統一。「今どこで話が切り替わったか」「今の話はどの仲間か」が構造で
   分かる (Ⅰ の中の 1 の (1)→(2) は同じ仲間、Ⅰ→Ⅱ は全く別の論点、という頭の動き)。
2. **宣言** — 文章をだらっと書かない・読まない。「今から定義」【定義】、【要件】①②③、
   【効果】…と中身に名前を付けてから書く。宣言セットは科目で変わる
   (法律=定義/要件/効果/論点/趣旨、英語は別系統)。

#### grill 確定 (R11-1〜R11-8、2026-06-11、全 8 問)

| # | 論点 | 確定 |
|---|---|---|
| R11-1 | 見出しの層 | **両方** — Ⅰ・1 = 冊のアウトライン (ピースの配置先)、(1)・① = ピース本文内。冊を開くと全ピースが見出し番号付きの **1 本の通し文書** に見える |
| R11-2 | アウトラインの作り手 | **AI が教材の構造 (まとまり/体系図) から下書き提案 → 子は言葉で直す**。ピースを刻むたび AI が配置先を提案 (OK or 言葉で変更)。数値・ドラッグ操作なし (M 系の「修正は言葉」と同じ) |
| R11-3 | 番号体系 | **Ⅰ→1→(1)→①→・ の 5 段を全科目・全冊で固定** (カスタム不可)。データはレベルで持ち表示時に必ずこの記法でレンダリング = 統一を仕組みで保証 |
| R11-4 | 宣言の出し方 | **パレット + AI 推奨 + 自作可のハイブリッド**。科目別既定チップ (タップ挿入) + AI がまとまりの中身を見て「使う宣言の並びだけ」提案 (中身は書かない = R3 整合) + 子の自作宣言は科目パレットに蓄積 |
| R11-5 | 添削との関係 | **3 色添削は構造も見る** (宣言と中身のズレ・階層の迷子を △ で方向指摘)。**ただし関所 (R8 理解済み判定) は中身基準のまま** — 構造で止めると整形ノルマに堕ちる |
| R11-6 | 入力 UX | **テキストエディタ維持 + AI 推奨テンプレ「この型で書く」ワンタップ挿入 (空テンプレを子が埋める) + 宣言チップ挿入**。音声 (R4) は従来どおり、葵の整形が宣言・番号記法に寄せる。構造化エディタは作らない |
| R11-7 | 冊の表示・移行 | **デフォルト = 通し文書ビュー** (番号付き 1 本の文書、ピースタップで詳細へ)。カード一覧・体系図は切替で残す。**既存ピースはアウトライン下書き時に AI が一括配置提案** (再入力なし) |
| R11-8 | 実装順 | **R11-① アウトライン基盤** (migration: 冊アウトライン + ピース配置参照 / AI 下書き / 既存ピース一括配置 / 通し文書ビュー) → **R11-② 書く型** (推奨テンプレ挿入 / 宣言パレット + 自作蓄積 / 刻む時の配置提案) → **R11-③ フィードバック** (添削の構造観点 / 音声整形の記法寄せ)。各段階で実機確認 |

**未決 (実装時に詰める)**: 科目別宣言セットの初期内容 (法律系/英語/数学/理社の具体チップ)、
アウトラインの深さの運用 (Ⅰ だけの小さい冊をどう見せるか)、通し文書の長大化対応 (折りたたみ等)。

#### R11-① 実装 (2026-06-11、✅migration `20260611010000` 本番適用済+REST 検証済)

- **DB**: `resumes.outline jsonb` = `[{id: "sec-1", title, entryIds[]}]`。章 = Ⅰ レベル、
  entryIds = 配下ピース (1 レベル) の表示順。**ピース配置はこの JSONB が唯一の置き場**
  (note_entries に列を足さない = 移動も並びも outline 上書きで完結)。null = 未生成。
- **AI 下書き**: `lib/notes/outline-claude.ts` (Server Action、Opus 4.8、text only)。
  入力 = 科目/冊名 + 全ピース (id/概念名/ページ/教材名) + 教材構造 (まとまり名列、章立てヒント)
  + (修正時) currentOutline + 子の指示。規律 = 全ピースを必ず 1 回だけ配置 / 章 2〜8 個 /
  修正は最小限の変更。出力検証 + 章 id 採番は client (`lib/notes/resume-outline.ts`
  sanitizeOutline: 実在しない・重複 entryId を除去)。
- **フォールバック** (`buildFallbackOutline`): 教材ごと 1 章・ページ順。flag off / 新規生成の
  失敗時。**修正の失敗は現状維持** (フォールバックで作り直すと子の指示を無視した全再編になる)。
- **通し文書ビュー** (`NotesHomeView`、デフォルト mode="doc"): 章見出し「Ⅰ タイトル」(下線) →
  配下ピース「1 概念名」+ 本文 (MarkdownText) + status アイコン + 出典 (教材名・ページ・読む) +
  open は「もう一回説明してみる」。**未整理セクション** = どの章にも居ないピース (新ピースは
  ここに入り「作り直す」で配置)。上部バー = アウトライン無し時「✨アウトラインを作る」CTA /
  有り時「言葉で直す」入力 + 直す + 作り直す。トグルは 通し/リスト/体系図 の 3 つ。
- 配線: `TutorWorkspace.handleGenerateOutline` (冊スコープのピース抽出は NotesHomeView の
  scopedEntries と同じ規則 = デフォルト冊は resume_id 未設定も拾う) → RightPaneRouter →
  NotesHomeView `onGenerateOutline`。
- **既知の割り切り**: 冊コピー (R10 Phase3) は outline をコピーしない (entryIds が複製前の
  id を指すため。コピー先で「作り直す」)。migration 未適用でも生成・表示は in-memory で動く
  (リロードで消えるだけ)。

#### R11-② 実装 (2026-06-11、書く型: 宣言パレット + この型で書く + 配置提案)

- **宣言パレット** (`lib/notes/declarations.ts`): 科目別の既定チップ —
  英語=ルール/形/意味/例文/例外/注意、数学=定義/公式/手順/例題/注意、
  国語=定義/ポイント/例/注意、理科=定義/しくみ/公式/実験/注意、
  社会=定義/背景/流れ/ポイント/注意、カスタム科目で名前に 法|税|簿記|会計 等 =
  **定義/趣旨/要件/効果/論点/例** (会計士レジュメの型)、その他汎用 = 定義/ポイント/理由/例/注意。
  自作宣言は「＋宣言を作る」で科目パレットに蓄積 (**MVP は localStorage**
  `ai-edu-declarations:{subjectId}`。端末またぎが要るなら DB 昇格)。
- **ResumePane**: Textarea の上にチップ行。タップでカーソル位置に【宣言】挿入
  (行の途中なら改行してから = だらっと続けない)。挿入は添削後なら writing に戻す。
- **「この型で書く」** (`lib/notes/template-claude.ts`、Opus 4.8 vision): 教材ページ +
  宣言パレットを渡し、**使う宣言の並びと番号枠 ((1)→①→・固定) だけの空テンプレ**を返す。
  中身・お手本・ヒント単語は書かせない (R3 整合を system で厳守)。本文が空なら置き、
  あれば末尾に追記。失敗 / flag off は `buildFallbackTemplate` (科目別固定 3 行型)。
- **刻む時の配置提案** (`outline-claude.suggestOutlinePlacement`、**Haiku 4.5** =
  ピースごとの高頻度なので segment-claude と同じコスト判断): `handleNoteAdded` で
  新規ピース + 入れ先の冊にアウトラインあり + flag on の時、章タイトル一覧から
  sectionIndex を判定して entryIds 末尾に自動配置 + DB 保存 + ゆい
  「レジュメの「Ⅱ ◯◯」に入れたよ📒 違ったら言葉で直してね」。
  合う章なし (-1) / 失敗は未整理のまま (「作り直す」か言葉修正で救う)。
#### R11-③ 実装 (2026-06-11、フィードバック: 添削の構造観点 + 「整える」。R11 三部作完結)

- **添削の構造観点** (`reviewResume` プロンプト拡張): 子が宣言・番号を使っている場合だけ、
  宣言と中身のズレ (【要件】の下に効果) と階層の迷子 (① の続きが別の話) を △ (missing) で
  方向だけ指摘 (多くて 1〜2 個)。宣言を使っていない本文に「宣言を使え」とは言わない
  (型の強制はしない)。**構造の指摘は「重大な△」に数えない = resolved (関所/R8) は
  中身基準のまま** (整形ノルマ化を防ぐ、grill R11-5 の通り)。
- **「整える」** (`note-gate-claude.tidyResumeBody`、Opus 4.8): 音声文字起こし (R4) や
  勢い書きの本文を、**子の言葉・内容・文順を一切変えずに**記法だけ整える — フィラー除去
  (えーと/あのー等)・句読点と改行・明らかな列挙を ①②・明らかな塊に【宣言】(パレットから、
  迷ったら付けない)・既存記法の (1)→①→・正規化。言い換え・要約・加筆・お手本は禁止 (R2)。
  失敗 / flag off は `tidyTranscriptFallback` (正規表現の簡易整形)。
- **ResumePane**: 🪄「整える」ボタン (「話す」の隣、本文空は disabled、実行前に録音停止)。
