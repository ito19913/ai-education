...
### 未確定論点 (残り 7 件、次セッション以降で grill)

| # | 論点 | 性格 |
|---|---|---|
| 1 | 失敗扱い | 重 (carry-over / 破綻許容、コーチング・ファースト整合の核) |
...
1. **失敗扱い** grill (最重要、コーチング・ファースト整合の核)
2. **中学生の主体性モデル** grill (小学生確定済、並列で)
3. **PlanType 5 種扱い** grill (現行 5 種をどうするか)
4. **カリキュラム DB の作成・運用** grill (ノード設計大転換の具体)
5. 残り 3 (系統可視性 / 時間予算 / 2 系統統合管制) はまとめて
